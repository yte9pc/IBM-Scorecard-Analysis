...
### linear assumptions for job level 1 subset
```{r}
summary(model.jobLevel1)
plot(model.jobLevel1, which=1:2)
boxcox(model.jobLevel1, lambda = seq(0, 1, by= 0.1))
```
...
### Linear assumptions for job level 2 subset
```{r}
...
### fit regression model for best r2 model for job level 3 subset
```{r}
...
### linear assumptions for job level 3 subset
```{r}
summary(model.jobLevel3)
plot(model.jobLevel3, which=1:2)
boxcox(model.jobLevel3, lambda = seq(0, 3, by= 0.1))
```

### box cox transformations
```{r}
model.jobLevel3.transform = lm(MonthlyIncome^2 ~ 1 + BusinessTravel + Education + JobRole + NumCompaniesWorked + RelationshipSatisfaction + StockOptionLevel + TotalWorkingYears, data=jobLevel3)
summary(model.jobLevel3.transform)

plot(model.jobLevel3.transform, which = 1:2)
qqline(model.jobLevel3.transform$residuals, col="red")
boxcox(model.jobLevel3.transform)
```

### Multicollinearity in job level 3 subset
```{r}
faraway::vif(model.jobLevel3.transform)
```

### Outliers Analysis in job level 3 subset
```{r}
##residuals
res = model.jobLevel3.transform$residuals 
##studentized residuals
student.res = rstandard(model.jobLevel3.transform) 
##externally studentized residuals
ext.student.res = rstudent(model.jobLevel3.transform) 

par(mfrow = c(1,3))
plot(model.jobLevel3.transform$fitted.values,res,main="Residuals")
plot(model.jobLevel3.transform$fitted.values,student.res,main="Studentized Residuals")
plot(model.jobLevel3.transform$fitted.values,ext.student.res,main="Externally  Studentized Residuals")

n = length(jobLevel3$MonthlyIncome)
p = length(coef(model.jobLevel3.transform))


##critical value using Bonferroni procedure
ext.student.res[abs(omit.na(ext.student.res)) > qt(1-0.05/(2*n), n-p-1)]
```
...
### fit regression model for best r2 model for job level 4 subset
```{r}
...
### linear assumptions for job level 4 subset
```{r}
summary(model.jobLevel4)
plot(model.jobLevel4, which=1:2)
boxcox(model.jobLevel4, lambda = seq(0, 3, by= 0.1))
```

### Multicollinearity in job level 4 subset
```{r}
faraway::vif(model.jobLevel4)
```
